# Hola, soy Pilar Olate

![Banner](https://capsule-render.vercel.app/api?type=waving&color=0:7F52FF,100:3DDC84&height=220&section=header&text=Android%20Developer%20Trainee&fontSize=34&fontColor=ffffff&animation=fadeIn&fontAlignY=38)

## Sobre mí
Soy Ingeniero Civil de profesión y actualmente estoy finalizando el curso de **Desarrollo de Aplicaciones Móviles Android Trainee**, donde he fortalecido mis habilidades en programación y desarrollo de aplicaciones móviles.

Me encuentro en transición hacia el mundo del desarrollo de software, con especial interés en la creación de aplicaciones Android funcionales, escalables y orientadas a resolver problemas reales.

## Intereses en tecnología
- Desarrollo de aplicaciones móviles Android
- Desarrollo de software
- Bases de datos relacionales
- Consumo de APIs
- Testing y mejora continua

## Tecnologías y herramientas

**Lenguajes y tecnologías**
<br>

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white)
![Android](https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

**Herramientas**
<br>

![Android Studio](https://img.shields.io/badge/Android%20Studio-3DDC84?style=for-the-badge&logo=androidstudio&logoColor=white)
![Apache NetBeans](https://img.shields.io/badge/Apache%20NetBeans-1B6AC6?style=for-the-badge&logo=apachenetbeanside&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)

## Formación reciente
Durante mi proceso formativo he trabajado en:

- Fundamentos de programación con **PSeInt** y **Java**
- Desarrollo de aplicaciones en **Apache NetBeans**
- Fundamentos de bases de datos relacionales con **MySQL**
- Desarrollo de interfaces de usuario en **Android Studio**
- Arquitectura y ciclo de vida de componentes Android
- Desarrollo de aplicaciones empresariales Android
- Consumo de APIs
- Pruebas y testing básico
- Desarrollo en **Java** y posteriormente en **Kotlin**

## Objetivo profesional
Seguir creciendo como desarrolladora Android, fortaleciendo mi experiencia en proyectos reales y ampliando mis conocimientos en desarrollo móvil, integración de servicios y buenas prácticas de programación.

## Proyectos e intereses
Me interesa seguir desarrollando:

- Aplicaciones móviles funcionales
- Proyectos que resuelvan problemas reales
- Aplicaciones conectadas a servicios web
- Soluciones tecnológicas con impacto práctico

## Contacto

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Perfil-0A66C2?style=for-the-badge&logo=linkedin)](https://linkedin.com/in/pilarolate)

[![GitHub](https://img.shields.io/badge/GitHub-pilarolate-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/pilarolate)

---
Gracias por visitar mi perfil.
